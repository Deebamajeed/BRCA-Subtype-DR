...
TCGAE = pd.read_csv("/content/gdrive/MyDrive/multiomics/protiens.csv",index_col=0)
TCGAEx=TCGAE.dropna(axis=1)
TCGAEx=TCGAEx.values
label_df=pd.read_csv("/content/data_clinical_patient_tcga_multi_protien_label.csv",index_col=0)
...
TCGAE = SelectKBest(chi2, k=600).fit_transform(x1, Y)
'''
ls_mb_size = [13, 36, 64]
    ...
    for repeat in range(10):
        for train_index, test_index in skf.split(TCGAE, Y.astype('int')):
            ...
            if k%50==0:
                ...
                #print("***********")
                #print(y_truet)
                #draw_cm(y_truet,predictiont)
                print(metrics.classification_report(y_truet,predictiont))
